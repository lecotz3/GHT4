...
Atualização: 11/09/2026. Piloto confirmado: **Distribuição e Trading Químico**, equilibrando mandatos de compra e venda. Esta matriz distingue implementação, configuração e aceitação pela GHT4. O plano completo ainda não está homologado.
...
| B11 — Linguagem natural | Conversa livre preparada; filtros explícitos e comparação de modelos | Conversão de linguagem natural em regras executáveis ainda pendente; observações não alteram filtros |
...
| B21 — Agente e saídas | Trabalhos, tarefas, acervo, PDF/DOCX/TXT/MD e Excel/PDF/JSON | Modelo real, revisão visual das telas novas e evolução para OCR/documentos maiores |
...
1. **Produto:** revisar acervo/modelos/backup no navegador quando a ferramenta de controle estiver disponível. Executar o roteiro com a GHT4 e corrigir obstáculos.
...
5. **Próximo desenvolvimento:** linguagem natural → prévia de filtros; atualização operacional de fontes; qualidade por campo; ranking calibrado; relatório setorial; documentos extensos; métricas por coorte. Esses itens continuam pendentes.
