...
OUT = BASE / 'GHT4 Planejamento de Prospeccao e Requisitos de IA.docx'
...
def footnote(source_id):
    ...
    if source_id==21:p.append(run(' Planos e diretórios do projeto GHT4; detalhes na referência 21.',size=18))
...
p=paragraph('Prospecção de mandatos na GHT4','Title')
pp=p.find(q('pPr'));sp=el('spacing',before=1080,after=360);pp.append(sp);body.append(p)
p=paragraph('Pesquisa e planejamento operacional com requisitos da ferramenta de IA')
p.find(q('pPr')).append(el('jc',val='center'));body.append(p)
meta=[['Tema','Originação e suporte a mandatos de compra e venda'],['Piloto','Distribuição e Trading Químico'],['Destinatários','Sócios, equipe Advisory e responsável técnico'],['Data de corte','10 de setembro de 2026'],['Versão','Planejamento proposto e requisitos consolidados']]
...
body.append(paragraph('Leitura sugerida: as seções 1 a 5 definem a estratégia e a operação; 6 a 13 especificam a ferramenta; 14 a 17 orientam implementação, aceite e gestão.'))
...
for node in core:
    ...
    if name=='title':node.text='Prospecção de mandatos na GHT4'
    if name in ('creator','lastModifiedBy'):node.text=''
    if name=='description':node.text='Planejamento operacional e requisitos de IA para prospecção M&A'
