...
function evidenciaDe(empresa, chave) {
  return EVIDENCIAS.evidenciaDe ? EVIDENCIAS.evidenciaDe(empresa, chave) : null;
}
...
function scorePapel(empresa, papel, config) {
  ...
  if (CONFIG.comoSinal) {
    for (const c of criterios) {
      const sinal = CONFIG.comoSinal(c);
...
function avaliarEmpresa(empresa, config) {
  ...
  return {
    ...
    criteriosNaoAvaliados: EVIDENCIAS.criteriosNaoAvaliados ? EVIDENCIAS.criteriosNaoAvaliados(empresa) : [],
    cobertura: EVIDENCIAS.coberturaEvidencia ? EVIDENCIAS.coberturaEvidencia(empresa, sinais) : null,
  };
}
