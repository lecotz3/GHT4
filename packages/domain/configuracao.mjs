...
const CAMPOS = {
   ...
   *
   * Lê da camada de conexões, que pode não estar disponível: o protótipo da
   * raiz não carrega `conexoes.js`. Por isso a guarda testa a CAPACIDADE
   * (`CONEXOES.forcaDe`) e não o módulo — no global gerado a ligação é tardia,
   * e um Proxy seria sempre verdadeiro. Sem a rede, o campo devolve null —
   * "não sei", que é a resposta certa, e nunca zero. */
  forcaConexao: {
    ...
    ler: (e) => (CONEXOES.forcaDe ? CONEXOES.forcaDe(e) : null),
  },
  temConexao: {
    ...
    natureza: 'cadastral',
    fonte: 'Cruzamento com currículos e listas de contatos da GHT4',
    ler: (e) => (CONEXOES.temVinculoRelevante ? CONEXOES.temVinculoRelevante(e) : null),
  },
...
function lerArmazenamento() {
  ...
  try {
    const cru = ARMAZENAMENTO.local().getItem(CHAVE_ARMAZENAMENTO);
...
function gravarArmazenamento(dados) {
  memoriaLocal = dados;
  try {
    ARMAZENAMENTO.local().setItem(CHAVE_ARMAZENAMENTO, JSON.stringify(dados));
    return true;
  } catch {
...
function configuracaoPadrao() {
  const pesos = {};
  const papeis = MOTOR.CONFIG_PAPEIS || {};
    ...
    for (const sinal in config.pesos[papel]) {
      ...
      if (de !== para) {
        mudancas.push({
          papel,
          sinal,
          rotulo: MOTOR.SINAIS && MOTOR.SINAIS[sinal] ? MOTOR.SINAIS[sinal].rotulo : sinal,
