...
function compradoresPara(alvo, avaliadas, opcoes) {
  ...
  for (const candidata of avaliadas) {
    ...
    for (const criterio of adicionais) {
      const resultado = CONFIG.avaliarCriterio
        ? CONFIG.avaliarCriterio(candidata, criterio)
        : null;
      if (resultado !== true) {
        reprovada = {
          empresa: candidata,
          criterio: CONFIG.descreverCriterio ? CONFIG.descreverCriterio(criterio) : criterio.campo,
...
function alvosPara(comprador, avaliadas, opcoes) {
  ...
  for (const candidato of avaliadas) {
    ...
    for (const criterio of adicionais) {
      const resultado = CONFIG.avaliarCriterio
        ? CONFIG.avaliarCriterio(candidato, criterio)
        : null;
      if (resultado !== true) {
        reprovado = {
          empresa: candidato,
